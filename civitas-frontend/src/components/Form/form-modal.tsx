    ...
    >
      <div className="shrink-0 border-b border-[var(--divider)] pb-5">
        <div className="flex flex-col">
          <h1 className="text-[24px] font-semibold text-[var(--foreground)] sm:text-[28px]">{title}</h1>
      ...
      <div className="mt-6 min-h-0 flex-1 overflow-y-auto pr-1">
        <div className="flex flex-col gap-4 pb-4">
          ...
          {extraContent ? <div className="border-t border-[var(--divider)] pt-4">{extraContent}</div> : null}
        </div>
      </div>

      <div className="mt-4 shrink-0 border-t border-[var(--divider)] pt-5">
        <div className="flex flex-col gap-3 md:flex-row">
          <Button variant="secondary" className="!w-full !max-w-none" onClick={onCancel} type="button">
