...
export default function ConfiguracoesFeedback({ feedback }: ConfiguracoesFeedbackProps) {
  ...
  return (
    <div className={`rounded-sm px-4 py-3 text-sm ${classes}`} role={feedback.type === "success" ? "status" : "alert"}>
      {feedback.message}
    </div>
  );
}
